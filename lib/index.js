...
class CookieRedisStore {
  constructor(redis, id) {
    ...
    debug('redis-cookie:redis:status')(this.redis.status);
    this.load((err, idx) => {
      if (err) throw err;
      if (idx) {
        this.idx = idx;
    ...
    while (!this.initialized || this.redis.status != "ready") { 
      debug('redis-cookie:redis:status')(this.redis.status);
      debug('redis-cookie:init')('loop');
      deasync.sleep(50);
    }
  }
  ...
  load(next) {
    ...
    debug('redis-cookie:redis')(this.redis.status);
    this.redis.get(this.id).then((data)=>{
      debug('redis-cookie:redis')('getdata');
      if (!data) return next();
